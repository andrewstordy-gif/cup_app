...
export function SessionDraftScreen({ onBackPress }) {
  const { width } = useWindowDimensions();
  const scale = Math.min(Math.max(width / 616, 0.58), 1.05);
  const [sessionName, setSessionName] = useState("Cup of Excellence");
  ...
  const [cuppingMode, setCuppingMode] = useState("blind");
  const cuppingModeRowRef = useRef(null);
  const [cuppingModeAnchor, setCuppingModeAnchor] = useState(null);
  const [samples, setSamples] = useState(DRAFT_SAMPLES);

  const deleteSample = (index) => setSamples(prev => prev.filter((_, i) => i !== index));

  const openDropdown = () => {
    typeRowRef.current?.measure((x, y, w, h, pageX, pageY) => {
      setAnchorRect({ x: pageX, y: pageY, width: w, height: h });
    });
  };
      ...
      <View style={styles.content}>
        <Header
          ...
          rightContent={
            !isDraft ? (
              <Pressable
                onPress={() => setIsDraft(true)}
                accessibilityRole="button"
                accessibilityLabel="Edit session"
                style={{ width: 44, height: 44, alignItems: "center", justifyContent: "center" }}
              >
                <AntDesign name="edit" size={22} color={colors.ink} />
              </Pressable>
            ) : undefined
          }
        />
        ...
        >
          <View style={[styles.metaList, { gap: 14 * scale }]}>
            ...
            <View style={{ paddingBottom: 14 * scale, gap: spacing.sm }}>
              ...
              <View style={styles.statusBadge}>
                <Text style={styles.statusBadgeText}>{isDraft ? "Draft" : "Pending"}</Text>
              </View>
            </View>
          </View>
          ...
          {samples.map((sample, index) => (
            <SampleCard
              ...
              index={index}
              scale={scale}
              onEdit={() => setAddSheetVisible(true)}
      ...
      <View onLayout={(e) => setFooterHeight(e.nativeEvent.layout.height)}>
        {isDraft ? (
          ...
          <ScreenFooter
            ...
            buttonStyle={{ backgroundColor: colors.action }}
          />
        )}
      </View>
