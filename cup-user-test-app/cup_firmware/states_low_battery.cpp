...
CupState run() {
  // LOW_BATTERY uses the same two-session NFC structure as OFF, but the debug
  // line is trimmed so it proves this state is specifically publishing and
  // reading back low-battery state.

  const uint8_t enteredState = gState.state;

  const unsigned long t0 = millis();

  // Session 1: publish the low-battery payload.
  drivers::nfc::begin();
  ...
  // Session 2: only read NDEF1/state back to decide whether to exit.
  drivers::nfc::begin();
  ...
  delay(100);
  Serial.print("low2,");
  Serial.print(enteredState);
  Serial.write(',');
  Serial.print((uint8_t)CupState::LOW_BATTERY);
  Serial.write(',');
  Serial.print(gState.state);
  Serial.write(',');
